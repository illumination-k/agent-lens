...
agent-lens analyze forwarding src/foo.rs --section wrapper
agent-lens analyze forwarding crates/agent-lens --format md   # wrappers + chains

# Declarations wider than their uses: functions one caller needs (with a
# calibration section for tuning --max-loc / --max-cyclomatic), abstractions
# one type implements, one-value parameters, over-exposed `pub`
agent-lens analyze narrowable crates/agent-lens --format md
agent-lens analyze narrowable crates/agent-lens --section single-use --format md

# Who reaches each function: untested, only tests keep alive, or unreachable
agent-lens analyze reach crates --format md
agent-lens analyze reach crates --section test-only --format md

# Tests that are near-copies of each other: which one to keep, and which
# of the rest are safe to fold into it
agent-lens analyze test-redundancy crates/agent-lens --format md
...
| `forwarding`      | Functions that only pass the call along, one section each: `wrapper` — a body that is a forwarding call modulo `?`, `.unwrap()`, `.into()`, `.await`, …, with argument-level evidence (Go interface-satisfying wrappers are annotated since deleting them isn't the fix); `delegation` — the chains those hops stack into (`api::save -> service::save -> repo::save`), with the terminus doing the work as the headline and a per-module delegator roll-up. Language-mandated hops are marked.                                                                                                                                                                                                                                          |
| `narrowable`      | Declarations wider than their uses, one section each: `single-use` — functions with exactly one resolved production caller, small and simple enough (`--max-loc`, `--max-cyclomatic`) to inline, with a calibration section and collapsible chains; `single-impl` — traits and interfaces with at most one production implementor, with mock-seam / `dyn` / visibility caveats and a removal-cost reference count; `parameters` — constant arguments and dead parameters, with a distinct-value calibration for `--min-call-sites`; `visibility` — `pub` / exported functions whose callers all sit in a narrower scope, with the declaration that would still compile. Every row is caveated where a hidden caller could make it wrong. |
| `test-redundancy` | Tests that are near-copies of each other, reduced to one keeper per set by a greedy dominating-set pass over the similarity graph. Each group is labelled `duplicate` or `parameterize` from how far the bodies agree beyond their shape, and the call graph subtracts what it can disprove: a pair whose tests exercise nothing in common is dropped, and a test that is the sole static caller of some function is never offered as foldable.                                                                                                                                                                                                                                                                                          |
...
| `reach`           | Who reaches each production function, one section per cell of reached-from-entries × reached-from-tests: `untested` — no resolved call path from any test, ranked by untested LOC; `test-only` — only tests keep it alive, candidates to move into test scope; `unreachable` — no entry point reaches it, in confidence tiers (`confirmed` / `likely` / `unknown`), sound in the "confirmed ⇒ really dead" direction only, with deletable islands and a deletion order.                                                                                                                                                                                                                                                                  |
...
- The `test-only` and `unreachable` sections of `reach`, and the
  `single-impl` and `visibility` sections of `narrowable`, judge Rust and
  Go only — the two adapters that extract export status (and, for
  `single-impl`, trait/interface declarations). `forwarding`'s `delegation`
  section runs everywhere
  but can only apply its module-facade exemption where export status exists.
  The `single-use` and `parameters` sections also run everywhere, but only
  Rust and Go rows can be caveat-free: TypeScript and Python rows always
  carry the unknown-export-status caveat.
- `test-redundancy` scores bodies in all four families, but its call-graph
  guard only speaks where the graph resolves the test's calls. A Rust test
...
an empty module, which matters most to `reach`'s `untested` section.
...
| `self-reach` | the workspace with tests kept — `reach` and `narrowable`: untested, unreachable, test-only, over-exposed code, single-caller and single-impl candidates |
| `self-tests` | the same crate, tests only — copy-pasted fixtures, dead helpers, near-copy tests to fold                                                                |
