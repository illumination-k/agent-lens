...
# Abstractions only one type implements, and production code only tests reach
agent-lens analyze single-impl crates/agent-lens --format md
agent-lens analyze test-only crates --format md
...
| `parameters`      | Parameters that only ever receive one thing: constant arguments (every resolved production call site passes the same literal, so the value can be inlined and the parameter dropped, with default-only Python/TS parameters as their own kind) and dead parameters the body never reads — caveated where a hidden call site could pass something else, with a distinct-value calibration section for setting `--min-call-sites` per repository. |
| `single-impl`     | Traits and interfaces with at most one production implementor — candidates for the concrete type — with mock-seam / `dyn` / visibility caveats, a removal-cost reference count, and the tree's implementor-count histogram.                                                                                                                                                                                                                     |
...
| `test-only`       | Production functions only tests keep alive — no production call path reaches them while a test does — plus public surface whose only callers are tests; candidates to move into test scope or delete with their tests.                                                                                                                                                                                                                          |
...
- `visibility`, `unreachable`, `test-only`, and `single-impl` judge Rust and
  ...
  `single-use` and `parameters` also run everywhere, but only Rust and Go
  rows can be caveat-free: TypeScript and Python rows always carry the
  unknown-export-status caveat.
...
| `self-reach` | the workspace with tests kept — untested, unreachable, test-only, over-exposed code, single-caller and single-impl candidates |
