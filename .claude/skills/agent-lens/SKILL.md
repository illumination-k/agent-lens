...
| Are there forwarding-only functions worth inlining?     | `forwarding --section wrapper`     | `.rs` / `.ts` / `.js` / `.py` / `.go` file or dir |
| How many hops of forwarding before the real work?       | `forwarding --section delegation`  | `.rs` / `.ts` / `.js` / `.py` / `.go` file or dir |
...
`similarity`, `forwarding`, `cohesion`, and `complexity` accept `--diff-only`, which restricts the report to functions or `impl` blocks touching unstaged changes (`git diff -U0`). Use this on a hot file rather than dumping the whole report into context.
...
agent-lens analyze forwarding crates/agent-lens/src --section delegation --format md
