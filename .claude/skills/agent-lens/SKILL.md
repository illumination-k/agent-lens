...
| How many layers does an error cross before handling?   | `error-chain`    | `.rs` / `.ts` / `.js` / `.py` / `.go` file or dir |
...
# Wrap chains: ranked caller→callee corridors where every function only
# propagates — check whether the middle layers add context or just re-wrap
agent-lens analyze error-chain crates/agent-lens --format md
...
- **error-chain**: each chain is a maximal caller→callee path of `wrap_only_error_path` functions — an error crosses `depth` layers before anything handles it. Long chains are not automatically bad (Go/Rust idiom encourages per-layer context); read the middle links and ask whether each hop adds new context (operation, arguments, domain translation) or just re-wraps the same message. Only heuristically-`resolved` call edges participate, so treat the report as a floor, not a census. Links marked `cycle` are mutual recursion collapsed into one step.
