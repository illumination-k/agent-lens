...
/// plus the path filter (only-tests / exclude-tests / glob excludes).
///
/// Held by analyzers as a single field so the four common builders can
/// be generated by [`delegate_filter_builders!`] instead of being
/// hand-written per analyzer.
...
impl FilterConfig {
    ...
    pub fn only_tests(&self) -> bool {
        self.path_filter.only_tests()
    }

    pub fn exclude_tests(&self) -> bool {
        self.path_filter.exclude_tests()
    }

    /// Borrow the inner [`AnalyzePathFilter`]. Lets analyzers hand the
    /// raw filter to helpers that need it directly (e.g. similarity's
    /// corpus collector).
