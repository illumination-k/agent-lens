...
/// `analyze graph-query` flags, and the `[profile.<name>.graph-query]`
/// table.
///
/// The one tool table with no defaults to fall back to: a traversal
/// without a verb and a start symbol has no meaning, so `query` and
/// `symbol` are required keys and [`crate::config::Config`] rejects a
/// profile that lists the tool without this table. Having no `Default` is
/// also why this type is written out rather than generated by
/// `analyzer_options!`.
...
pub struct GraphQueryOptions {
    ...
    #[arg(long)]
    pub symbol: String,
    /// Destination symbol for the `path` query (same matching rules as
    /// the start symbol).
    #[arg(long)]
    ...
    /// Traversal direction for the `neighborhood` query (default both).
    #[arg(long, value_enum)]
...
impl GraphQueryAnalyzer {
    ...
    /// Build from a whole [`GraphQueryOptions`] group. The CLI flags and
    /// the `[profile.<name>.graph-query]` table are the same type, so this
    /// is the only seam between parsed options and the analyzer. Unlike
    /// the other analyzers this constructs rather than configures —
    /// `query` and `symbol` are required.
    pub fn from_options(opts: GraphQueryOptions) -> Self {
        Self::new(opts.query, opts.symbol)
            .with_to(opts.to)
            .with_depth(opts.depth)
            .with_direction(opts.direction)
