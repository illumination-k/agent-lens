...
/// Everything [`candidate_pairs`] needs beyond the corpus and its tree
/// profiles. Grouped rather than passed positionally: the run
/// configuration is threaded through verbatim and six same-typed
/// booleans and floats in a row are easy to mis-order at the call site.
#[derive(Debug, Clone, Copy)]
pub(super) struct CandidateConfig<'a> {
    pub min_lines: usize,
    /// Body-score threshold the cheap TSED filters prune against.
    pub threshold: f64,
    pub opts: &'a TSEDOptions,
    pub method: SimilarityMethod,
    pub allow_lsh: bool,
    /// Drop pairs whose source spans overlap in one file. Only the
    /// blocks target needs this; its sliding windows overlap by
    /// construction.
    pub exclude_overlapping: bool,
}
...
pub(super) fn candidate_pairs(
    ...
    let (pairs, filter_counts) = if use_lsh {
        ...
        let lsh_pairs = lsh_candidate_pairs_for_trees(&trees, &strategy.lsh)
            ...
            .filter(|(i, j)| !exclude_overlapping || !spans_overlap(corpus, *i, *j));
        match method {
...
/// Whether two units cover overlapping source lines in one file.
///
/// Block windows are generated by sliding over statement lists, so the
/// runs starting at statement 0 and at statement 1 share all but one
/// statement and would otherwise be reported as a near-perfect clone of
/// each other. That is an artefact of the windowing, not duplication a
/// reader can act on.
fn spans_overlap(corpus: &[OwnedUnit], i: usize, j: usize) -> bool {
    ...
    fn cartesian_candidates_never_include_self_pairs() {
        ...
        let candidates = candidate_pairs(
            ...
            &CandidateConfig {
                min_lines: 1,
                threshold: 0.0,
                opts: &lens_domain::TSEDOptions::default(),
