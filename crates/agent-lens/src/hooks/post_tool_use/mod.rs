...
impl HookEnvelope for ClaudeCodePostToolUse {
    ...
    /// missing `file_path`, or an extension the analysers can't handle.
    /// The list is at most one element long; returning a `Vec` lets the
    /// engine-agnostic core treat Claude Code and Codex inputs the same way.
