...
//! - [`items`]: walk `syn::Item` trees and emit module-aware function defs.
//! - [`types`]: collect type-path tokens from `syn::Type` trees.
//! - [`patterns`]: extract bound identifier names from `syn::Pat` trees.
//! - [`mod@self`]: public API plus the tree projection used by APTED.
...
use items::extract_module_functions;
use patterns::collect_pattern_names;
use types::collect_type_paths;
